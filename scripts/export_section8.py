...
def reconcile_table(data: dict) -> str:
    ...
    colspec = (
        ...
        r" >{\raggedright\arraybackslash}p{8.35cm}@{}"
    )
    rows = [
        (
        ...
        (
            ...
            f"(simultaneous unique heads; organic $=$ "
        ...
        (
            ...
            f"A-134 $6\\to 1$~d; A-141 stays 5~d; A-142 stays 4~d & "
            f"Not in Table~\\ref{{tab:cashflow}}; Board draw under the "
            f"Sec.~\\ref{{sec:delivery-governance}} DoA \\\\"
        ),
        (
            f"Gate~1 PV & {aud(proj['pv_at_gate1'])} already planned by 2~Oct "
            f"(40.2\\% of work PMB) & "
            f"Sec.~\\ref{{sec:exec}}; Sec.~\\ref{{sec:cost-basis}}; "
            f"Fig.~\\ref{{fig:scurve}} \\\\"
        ),
        (
            "A-100 & External \\$0 freight driver; not a launch-critical Gilmour bar & "
            "Table~\\ref{tab:precedence} CP $=$ Ext; Fig.~\\ref{fig:network_logic}; "
            "Fig.~\\ref{fig:gantt_chart} \\\\"
        ),
        (
            "Crane weather & Only RES 4 extra wet days $\\times$ \\$4{,}200 $=$ \\$16{,}800; "
            "1.2.2 16~d are weld/NDT/bolt-up; R-06 EMV \\$0 & "
            "Table~\\ref{tab:wbs-cost}; Table~\\ref{tab:contingency}; R-06 \\\\"
        ),
        (
